...
export class BroadcastCommand extends OfficerCommand {
    constructor(client: CommandoClient) {
        super(client, {
            aliases: ["bcr", "broadcastreserve"],
            args: [
                {
                    default: "",
                    key: "message",
                    prompt: "Give the message to send to the reserves.",
                    type: "string",
                },
            ],
            description: "Pings all reserves of a raid with a message.",
            group: "raids",
            guildOnly: true,
            memberName: "broadcastreserves",
            name: "broadcastreserves",
        });
    }
    ...
    public async run(message: CommandMessage,
        ...
        const users = ev.roles.flatMap(role => role.getParticipantsByStatus("reserve").map(p => p.userId));
